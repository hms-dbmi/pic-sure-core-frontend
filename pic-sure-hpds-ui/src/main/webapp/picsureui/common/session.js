define(["jquery", "underscore", "overrides/session", "picSure/settings", "common/styles", 'common/transportErrors'],
    function ($, _, sessionOverrides, settings, styles, transportErrors) {
            ...
            $.ajaxSetup({
                ...
                complete: function (xhr) {
                    ...
                    if (authorizationHeader && authorizationHeader.startsWith('Bearer ')) {
                        ...
                        session.token = authorizationHeader.substring(7).trim();
        ...
        let authenticated = function (token, username, __unused_permissions, acceptedTOS, oktaIdToken) {
            session.token = token;
            session.username = username;
            session.acceptedTOS = acceptedTOS;
            session.email = username;
            session.oktaIdToken = oktaIdToken;
            sessionStorage.setItem("session", JSON.stringify(session));
            configureAjax();
        };
        ...
        return {
            ...
            sessionInit: function (data) {
                authenticated(data.token, data.email, data.permissions, data.acceptedTOS, data?.oktaIdToken, (sessionOverrides.handleNotAuthorizedResponse ? sessionOverrides.handleNotAuthorizedResponse : handleNotAuthorizedResponse));
