define([
	...
	"backbone",
	"handlebars",
	"text!filter/filter.hbs",
	"text!filter/suggestion.hbs",
	"text!filter/noResults.hbs",
	"filter/searchResults",
	"text!filter/constrainFilterMenu.hbs",
	"text!filter/constrainFilterMenuCategories.hbs",
	"text!filter/constrainFilterMenuGenetics.hbs",
	"text!filter/constrainFilterMenuVariantInfoNumeric.hbs",
	...
	var filterView = BB.View.extend({
	  initialize: function (opts) {
		...
		this.onSelect = overrides.onSelect
		  ? overrides.onSelect.bind(this)
		  : this.onSelect.bind(this);
		this.render = overrides.render
		  ? overrides.render.bind(this)
	  ...
	  events: {
		...
		"click .constrain-genetics-dropdown-menu li a":
		  "onConstrainGeneticsSelect",
		...
		"click #search-button": "enterButtonEventHandler",
	  ...
	  enterButtonEventHandler: function (event) {
		...
		if (event.keyCode == 13 || event.type == "click") {
		  ...
		  overrides.enterKeyHandler
			? overrides.enterKeyHandler.apply(this)
			: function () {
	  ...
	  onDropdownSelect: function (event) {
		var dropdownElement = $(
		  "." +
			event.target.parentElement.parentElement.attributes["aria-labelledby"]
			  .value,
		  this.$el
		);
	  ...
	  onSelect: function (event, suggestion) {
		if (this.model.attributes.valueType === "ANYRECORDOF") {
		  ...
		  this.model.set(
			"inclusive",
			$(".filter-qualifier-btn", this.$el).text().trim() === "Must Have"
		  );
		  this.model.set(
			"and",
			$(".filter-boolean-operator-btn", this.$el).text().trim() === "AND"
		  );
	  ...
	  onConstrainTypeSelect: function (event) {
		...
		this.model
		  .get("constrainParams")
		  .set("constrainByValue", constrainByValue);
	  ...
	  onConstrainGeneticsSelect: function (event) {
		...
		this.model
		  .get("constrainParams")
		  .set("constrainValueOne", this.model.attributes.searchTerm);
		this.model
		  .get("constrainParams")
		  .set("constrainValueTwo", event.target.text);
		this.model.attributes.valueType = "VARIANT";
		this.updateConstrainFilterMenu();
	  },
	  ...
	  selectAllCategories: function (event) {
		...
		if (existingItems.length == 0) {
		  $(".selected-categories", this.$el).append(
			$(".available-categories > option:visible", this.$el)
		  );
		  return;
		}
		...
		$(".available-categories > option:visible", this.$el).each(function () {
		  ...
		  if (this.text < currentItem.text()) {
			...
			while (
			  currentItem.next().length > 0 &&
			  this.text > currentItem.next().text()
			) {
	  ...
	  clearCategorySelection: function (event) {
		...
		if (existingItems.length == 0) {
		  $(".available-categories", this.$el).append(
			$(".selected-categories > option", this.$el)
		  );
		  return;
		}
	  ...
	  onConstrainCategorySelect: function (event) {
		...
		if (
		  event.target.parentElement.classList.contains("available-categories")
		) {
	  ...
	  onConstrainValuesChange: function (event) {
		this.model
		  .get("constrainParams")
		  .set("constrainValueOne", $(".constrain-value-one", this.$el).val());
		this.model
		  .get("constrainParams")
		  .set("constrainValueTwo", $(".constrain-value-two", this.$el).val());
	  },
	  ...
	  updateConstrainFilterMenu: function () {
		...
		if (this.model.attributes.valueType === "ANYRECORDOF") {
		  ...
		  this.model.attributes.concept.columnDataType === "CONTINUOUS"
		) {
		  filterEl.html(
			this.constrainFilterMenuTemplate(
			  _.extend(
				this.model.attributes.constrainParams.attributes,
				this.model.attributes.concept
			  )
			)
		  );
		  if (this.model.attributes.constrainParams.attributes.constrainByValue) {
			this.updateConstrainValueVisibility(
			  this.model.attributes.constrainParams.attributes.valueOperator
			);
		  }
		} else if (this.model.attributes.concept.columnDataType === "VARIANT") {
		  filterEl.html(
			this.constrainFilterMenuGeneticsTemplate(
			  _.extend(
				this.model.attributes.constrainParams.attributes,
				this.model.attributes.concept
			  )
		  ...
		  this.model.attributes.concept.columnDataType === "INFO" &&
		  this.model.attributes.concept.metadata.continuous
		) {
		  ...
		  filterEl.html(
			this.constrainFilterMenuVariantInfoNumericTemplate(
			  _.extend(
				this.model.attributes.constrainParams.attributes,
				this.model.attributes.concept
			  )
		  ...
		  filterEl.html(
			this.constrainFilterMenuCategoriesTemplate(
			  _.extend(
				this.model.attributes.constrainParams.attributes,
				this.model.attributes.concept
			  )
		  ...
		  var selectedCategories = this.model
			.get("constrainParams")
			.get("constrainValueOne");
		  $(".available-categories > option", this.$el).each(
			function (idx, option) {
			  if (
				(Array.isArray(selectedCategories) &&
				  selectedCategories.includes(option.text)) ||
	  ...
	  validateConstrainFilterFields: function () {
		...
		if (
		  constrainParams.get("constrainByValue") &&
		  this.model.attributes.concept.columnDataType === "CONTINUOUS"
		) {
		  ...
		  if (
			constrainParams.get("valueOperator") == "LT" ||
			constrainParams.get("valueOperator") == "GT"
		  ) {
			if (
			  constrainValueOne == "" ||
			  isNaN(constrainValueOne) ||
			  !this.isValueInRange(
				constrainValueOne,
				this.model.attributes.concept.metadata.min,
				this.model.attributes.concept.metadata.max
			  )
			) {
	  ...
	  updateModel: function () {
		...
		if (
		  this.model.get("concept").columnDataType == "CATEGORICAL" ||
		  (this.model.get("concept").columnDataType == "INFO" &&
			!this.model.get("concept").metadata.continuous)
		) {
		  if ($(".category-filter-restriction", this.$el).val() == "RESTRICT") {
			this.model
			  .get("constrainParams")
			  .set(
				"valueOperatorLabel",
				$(".category-filter-restriction option:selected", this.$el).text()
			  );
			...
			this.model
			  .get("constrainParams")
			  .set("constrainValueOne", selectedCategories);
		  } else {
	  ...
	  onConstrainApplyButtonClick: function (event) {
		...
		if (this.validateConstrainFilterFields()) {
		  ...
		  if (this.model.attributes.concept.columnDataType === "INFO") {
			...
			$(".search-value", this.$el).html(
			  "Variant Info Column " +
				this.model.get("category") +
				": " +
				this.model.get("constrainParams").get("constrainValueOne")
			);
			...
			this.model
			  ...
			  .set(
				"constrainValueTwo",
				$(".value-constraint-genetics-btn", this.$el).text()
			  );
			$(".search-value", this.$el).html(
			  this.model.get("constrainParams").get("constrainValueTwo") +
				" : " +
				this.model.get("searchTerm")
			);
		  } else {
			var constrains = this.model.get("constrainParams");
			if (
			  constrains.get("constrainByValue") &&
			  !(
			  ...
			  var searchParam =
				...
				" " +
				(Array.isArray(constrains.get("constrainValueOne"))
				...
				constrains.get("constrainValueTwo");
			  $(".search-value", this.$el).html(
				this.model.get("searchValue") + ", " + searchParam
			  );
			} else {
			  $(".search-value", this.$el).html(
				this.model.get("searchValue") + ", Any Value"
			  );
			}
		  }
		  ...
		  $(".validation-message", this.$el).text(
			"Value invalid! Correct invalid fields."
		  );
		}
	  },
	  ...
	  geneticSelections: function (searchString) {
		if (
		  /\d+,\d+,.*,.*/.test(searchString) ||
		  /\d+:\d+_.*/.test(searchString)
		) {
	  ...
	  render: function () {
		...
		if (this.model.attributes.valueType === "ANYRECORDOF") {
		  $(".category-valueof-div", this.$el).html(
			this.constrainFilterMenuAnyRecordOfTemplate(
			  this.model.attributes.anyRecordCategories
			)
		  );
		}
